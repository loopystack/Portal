...
router.get('/entries', (0, express_validator_1.query)('from').isISO8601(), (0, express_validator_1.query)('to').isISO8601(), (0, express_validator_1.query)('userId').optional().trim().notEmpty(), async (req, res) => {
    try {
        ...
        const { rows } = await pool_1.pool.query(`SELECT id, user_id, amount, date, note, created_at, updated_at
         ...
         ORDER BY date DESC, created_at DESC`, [userId, from, to]);
...
router.post('/entries', (0, express_validator_1.body)('date').isISO8601(), (0, express_validator_1.body)('amount').isFloat().toFloat(), (0, express_validator_1.body)('note').optional().trim().isLength({ max: 2000 }), async (req, res) => {
    try {
        ...
        const { rows } = await pool_1.pool.query(`INSERT INTO revenue_entries (user_id, date, amount, note)
         VALUES ($1, $2, $3, $4)
         RETURNING id, user_id, amount, date, note, created_at, updated_at`, [userId, date, amount, note || null]);
...
router.delete('/entries/:id', (0, express_validator_1.param)('id').isUUID(), async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            res.status(400).json({ errors: errors.array() });
...
router.get('/expected', (0, express_validator_1.query)('year').isInt({ min: 2000, max: 2100 }).toInt(), (0, express_validator_1.query)('month').isInt({ min: 1, max: 12 }).toInt(), async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            res.status(400).json({ errors: errors.array() });
        ...
        const userId = req.user.userId;
        const { rows } = await pool_1.pool.query(`SELECT id, user_id, month, year, amount, created_at, updated_at
...
router.put('/expected', (0, express_validator_1.body)('year').isInt({ min: 2000, max: 2100 }).toInt(), (0, express_validator_1.body)('month').isInt({ min: 1, max: 12 }).toInt(), (0, express_validator_1.body)('amount').isFloat({ min: 0 }).toFloat(), async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            res.status(400).json({ errors: errors.array() });
        ...
        const { rows } = await pool_1.pool.query(`INSERT INTO expected_revenue (user_id, year, month, amount)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (user_id, month, year)
         DO UPDATE SET amount = $4, updated_at = NOW()
         RETURNING id, user_id, month, year, amount, created_at, updated_at`, [userId, year, month, amount]);
